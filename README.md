...
- реакции («tapback» эмодзи) на mesh-сообщения дописываются строкой `reactions: …` в исходный форвард через редактирование сообщения (нативные Telegram-реакции не используются — бот может поставить только одну и перезаписывал бы предыдущие);
